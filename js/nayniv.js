...
import { playThunder2Sound, playKanatSound } from './sound.js';
...
export function initNaymAnimation() {
    ...
    function onSpineEvent(entry, event) {
        console.log('Naym Spine event получен:', entry, event);
        ...
        if (eventName === 'grom') {
            const now = Date.now();
            if (now - lastGromTime >= SOUND_COOLDOWN) {
                lastGromTime = now;
                playKanatSound();
                console.log('Воспроизводим звук каната (kanat)');
            }
        } else if (eventName === 'lightning') {
            const now = Date.now();
            if (now - lastLightningTime >= SOUND_COOLDOWN) {
                lastLightningTime = now;
                playThunder2Sound();
                console.log('Воспроизводим звук молнии (thunder2)');
            }
        }
    }
    ...
    app.loader
        ...
        .load((loader, resources) => {
            ...
            setTimeout(() => {
                ...
                // Добавляем обработчик событий Spine
