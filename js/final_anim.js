...
import { playStrikeSound, playTimeSound } from './sound.js';

export function initFinalAnimation() {
    let container = document.querySelector('.anim_final');
    
    if (!container) {
        console.error('Контейнер .anim_final не найден');
        return null;
    }
    ...
    // Обработчик событий Spine
    function onSpineEvent(entry, event) {
        console.log('Final Spine event получен:', entry, event);
        ...
        if (eventName === 'strike') {
            const now = Date.now();
            if (now - lastStrikeTime >= SOUND_COOLDOWN) {
                lastStrikeTime = now;
                playStrikeSound();
                console.log('Воспроизводим звук strike');
            }
        } else if (eventName === 'time') {
            const now = Date.now();
            if (now - lastTimeTime >= SOUND_COOLDOWN) {
                lastTimeTime = now;
                playTimeSound();
                console.log('Воспроизводим звук time');
            }
        }
    }
    ...
    scaleContainer.className = 'final-scale-container';
    
    // SVG шкалы (вы вставите свой SVG код)
    ...
    // Здесь вы вставите свой SVG код
    ...
    fillRect.classList.add('final-fill');
    ...
    // Функция для скрытия шкалы
    function hideScale() {
        scaleContainer.style.transition = 'opacity 0.5s ease';
    ...
    // ========== ПАРАМЕТРЫ ШКАЛЫ ==========
    const minHandleY = 32;
    const maxHandleY = 220;
    const handleHeight = 35;
    const handleRadius = handleHeight / 2;
    
    // ========== КРУГ ДЛЯ ПЕРЕТАСКИВАНИЯ ==========
    const handle = document.createElement('div');
    handle.className = 'final-handle';
    handle.style.touchAction = 'none';
    scaleContainer.appendChild(handle);
    
    // ========== ОБРАБОТЧИКИ ДЛЯ ТЕЛЕФОНА ==========
    handle.addEventListener('touchstart', (e) => {
        if (isStrikePlaying || isEndPlaying) return;
        e.preventDefault();
        e.stopPropagation();
    ...
    document.addEventListener('touchmove', (e) => {
        if (!isDragging || isStrikePlaying || isEndPlaying) return;
        e.preventDefault();
    ...
    function updateProgressFromTop(top) {
        ...
        // Если достигли нижней точки - запускаем strike анимацию
        if (currentProgress >= 99 && !isStrikePlaying && !isEndPlaying && spineCharacter) {
            triggerStrike();
        }
    }
    ...
    function triggerStrike() {
        ...
        // Запускаем strike анимацию (один раз)
        spineCharacter.state.setAnimation(0, "strike", false);
    }
    ...
    function triggerEnd() {
        ...
        isEndPlaying = true;
        // Запускаем end анимацию (зацикленная)
        spineCharacter.state.setAnimation(0, "end", true);
    }
    ...
    app.loader
        .add("spine", "assets/animations/final/final.json")
        .load((loader, resources) => {
            spineCharacter = new PIXI.spine.Spine(resources.spine.spineData);
            window.finalSpine = spineCharacter;
            ...
            setTimeout(() => {
                ...
                console.log('Final animation bounds:', originalBounds);
                
                updateSizeAndPosition();
                app.stage.addChild(spineCharacter);
                
                // Настройка переходов между анимациями
                spineCharacter.state.data.setMix("idle", "strike", 0.3);
                spineCharacter.state.data.setMix("strike", "end", 0.3);
                spineCharacter.state.data.setMix("end", "idle", 0.3);
                ...
                spineCharacter.state.addListener({
                    complete: (entry) => {
                        if (entry.animation.name === "strike" && isStrikePlaying) {
                            // После strike запускаем end и скрываем шкалу
...
export function removeFinalAnimation() {
    const container = document.querySelector('.anim_final');
    if (container) {
        container.innerHTML = '';
        container.style.minHeight = '';
    }
    window.finalSpine = null;
}